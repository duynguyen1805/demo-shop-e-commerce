...
import Cart_Order from "./cart/cart-order";
import Cart_Notification from "./cart/cart-notification";
...
const Header = () => {
  ...
  const [isOpen_Notification, setIsOpen_Notification] = useState(false);
  const [isOpen_Cart, setIsOpen_Cart] = useState(false);
  const [isOpen_Account, setIsOpen_Account] = useState(false);
  ...
  const handleLogout = async () => {
    ...
    const userData = localStorage.getItem("user_shopee");
    const isLoginData = localStorage.getItem("isLogin");

    // Kiểm tra nếu dữ liệu tồn tại
    if (userData && isLoginData) {
      // Xóa các trường user và isLogin
      localStorage.removeItem("user_shopee");
    ...
    <>
      <div className="h-[84px] w-full flex items-center justify-center bg-main_color bg-opacity-90 backdrop-filter backdrop-blur-sm sticky top-0 z-50">
        <div className="h-full w-[1200px] flex items-center place-content-between">
          ...
          {/* NOTIFICATION */}
          <Popover
            open={isOpen_Notification}
            onOpenChange={setIsOpen_Notification}
          >
            <PopoverTrigger>
              <div
                className="h-auto w-[120px] flex items-center justify-center gap-1 cursor-pointer"
                onMouseEnter={() => setIsOpen_Notification(true)}
              >
                <Image
            ...
            <PopoverContent className="mt-2">
              <div className="relative h-auto max-h-[505px] min-h-[168px] w-[500px] rounded flex flex-col overflow-auto">
                <div className="min-h-14 w-full px-3 flex items-center place-content-between border-b-4 sticky top-0 bg-white">
          ...
          {/* CART */}
          <Popover open={isOpen_Cart} onOpenChange={setIsOpen_Cart}>
            <PopoverTrigger>
              <div
                className="relative h-auto w-[120px] flex items-center justify-center gap-1 cursor-pointer"
                onMouseEnter={() => setIsOpen_Cart(true)}
              >
                <Image
                  src={icon_cart}
                  alt=""
                ...
                <span className="absolute top-[-12px] left-6 w-6 rounded-full bg-red-500 text-white flex items-center justify-center">
                  0
                </span>
              </div>
            </PopoverTrigger>
                  ...
                  cart_order.map((item, index) => {
                    return (
                      <div key={index} className="">
                        <Cart_Order
                          item={item}
                          setIsOpen_Cart={() => setIsOpen_Cart(false)}
                        />
                      </div>
                    );
                  })}
          ...
          {user !== null ? (
            <>
              <Popover open={isOpen_Account} onOpenChange={setIsOpen_Account}>
                <PopoverTrigger>
                  <div
                    className="h-auto w-[170px] flex items-center justify-center gap-2 px-1 cursor-pointer overflow-hidden"
                    onMouseEnter={() => setIsOpen_Account(true)}
                  >
                    {user && user.avatar ? (
                      <div
                        className="h-[50px] w-[50px] bg-cover bg-no-repeat bg-center rounded-full"
                        style={{
                          backgroundImage: `url(${
                ...
                <PopoverContent className="mt-2">
                  <div className="min-h-[160px] h-auto w-48 rounded flex flex-col text-lg">
                    <Link
                      href={"/admin"}
            ...
            <div className="h-[50px] w-auto flex flex-col items-start text-white">
              <span
